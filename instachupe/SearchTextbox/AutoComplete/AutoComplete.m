...
@property (strong) id <AutoCompleteDelegate> delegate;
@property (strong) NSDictionary *boldTextAttributes;
@property (strong) NSDictionary *regularTextAttributes;

- (id)initWithDelegate:(id<AutoCompleteDelegate>)aDelegate
...
@property (strong) SearchTextbox *textField;
@property (strong) id <AutoCompleteDelegateFetchAction> delegate;
@property (strong) id <DataSource> dataSource;

- (id)initWithDelegate:(id<AutoCompleteDelegateFetchAction>)aDelegate
 completionsDataSource:(id<DataSource>)aDataSource
 AutoCompletetxt:(SearchTextbox *)aTextField;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    } else if ([autoCompleteObject conformsToProtocol:@protocol(CompletionObject)]){
        suggestedString = [(id <CompletionObject>)autoCompleteObject autocompleteString];
    } else {
        NSAssert(0, @"Autocomplete suggestions must either be NSString or objects conforming to the CompletionObject protocol.");
    }
...
withAutoCompleteString:(NSString *)string
{
    ...
    id autoCompleteObject = self.autoCompleteSuggestions[indexPath.row];
    if(![autoCompleteObject conformsToProtocol:@protocol(CompletionObject)]){
        autoCompleteObject = nil;
    }
    
    if([self.autoCompleteDelegate respondsToSelector:@selector(AutoCompletetxt:shouldConfigureCell:withAutoCompleteString:withAttributedString:forAutoCompleteObject:forRowAtIndexPath:)])
    {
        if(![self.autoCompleteDelegate AutoCompletetxt:self shouldConfigureCell:cell withAutoCompleteString:string withAttributedString:boldedString forAutoCompleteObject:autoCompleteObject forRowAtIndexPath:indexPath])
        {
            return;
        }
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    id<CompletionObject> autoCompleteObject = self.autoCompleteSuggestions[indexPath.row];
    if(![autoCompleteObject conformsToProtocol:@protocol(CompletionObject)]){
        autoCompleteObject = nil;
    }
    
    if([self.autoCompleteDelegate respondsToSelector:
        @selector(AutoCompletetxt:didSelectAutoCompleteString:withAutoCompleteObject:forRowAtIndexPath:)]){
        isSelectedToDismiss=YES;
        [self.autoCompleteDelegate AutoCompletetxt:self
...
- (BOOL)resignFirstResponder
{
    ...
    if(!isSelectedToDismiss && self.autoCompleteShouldSelectOnExactMatchAutomatically){
        ...
        if(isContains){
            ...
            if([self.autoCompleteDelegate respondsToSelector:@selector(AutoCompletetxt:didResignWithExactMatch:)]){
            [self.autoCompleteDelegate AutoCompletetxt:self didResignWithExactMatch:strSelected];
            }
            [self closeAutoCompleteTableView];
        }
    }else{
    ...
    if(numberOfRows && (self.autoCompleteTableViewHidden == NO)){
        ...
        if(!self.autoCompleteTableView.superview){
            if([self.autoCompleteDelegate
                respondsToSelector:@selector(AutoCompletetxt:willShowAutoCompleteTableView:)]){
                [self.autoCompleteDelegate AutoCompletetxt:self
...
- (void)closeAutoCompleteTableView
{
    ...
    if([self.autoCompleteDelegate respondsToSelector:@selector(AutoCompletetxt:willHideAutoCompleteTableView:)]){
        [self.autoCompleteDelegate AutoCompletetxt:self willHideAutoCompleteTableView:self.autoCompleteTableView];
    }
    [self restoreOriginalShadowProperties];
...
{
    if([self.autoCompleteDelegate respondsToSelector:@selector(AutoCompletetxt:shouldStyleAutoCompleteTableView:forBorderStyle:)]){
        if(![self.autoCompleteDelegate AutoCompletetxt:self
...
- (UITableView *)newAutoCompleteTableViewForTextField:(SearchTextbox *)textField
{
...
- (CGRect)autoCompleteTableViewFrameForTextField:(SearchTextbox *)textField
                                 forNumberOfRows:(NSInteger)numberOfRows
{
    ...
    if([self.autoCompleteDelegate respondsToSelector:@selector(AutoCompletetxt:HeightDidChange:)]){
        [self.autoCompleteDelegate AutoCompletetxt:self HeightDidChange:height];
    }
...
- (CGFloat)autoCompleteTableHeightForTextField:(SearchTextbox *)textField
...
- (void)main
{
    @autoreleasepool {
        ...
        if([self.dataSource respondsToSelector:@selector(AutoCompletetxt:possibleCompletionsForString:completionHandler:)]){
            __weak MLPAutoCompleteFetchOperation *operation = self;
            sentinelSemaphore = dispatch_semaphore_create(0);
            [self.dataSource AutoCompletetxt:self.textField
        ...
        } else if ([self.dataSource respondsToSelector:@selector(AutoCompletetxt:possibleCompletionsForString:)]){
            
            NSArray *results = [self.dataSource AutoCompletetxt:self.textField
                                possibleCompletionsForString:self.incompleteString];
            ...
            NSAssert(0, @"An autocomplete datasource must implement either AutoCompletetxt:possibleCompletionsForString: or AutoCompletetxt:possibleCompletionsForString:completionHandler:");
        }
    ...
    if(!self.isCancelled){
        ...
        if(suggestions.count){
            ...
            NSAssert([firstObject isKindOfClass:[NSString class]] ||
                     [firstObject conformsToProtocol:@protocol(CompletionObject)],
                     @"SearchTextbox expects an array with objects that are either strings or conform to the CompletionObject protocol for possible completions.");
        }
...
- (NSArray *)sortedCompletionsForString:(NSString *)inputString withPossibleStrings:(NSArray *)possibleTerms
{
    ...
    for(NSObject *originalObject in possibleTerms) {
        ...
        } else if ([originalObject conformsToProtocol:@protocol(CompletionObject)]){
            currentString = [(id <CompletionObject>)originalObject autocompleteString];
        } else {
